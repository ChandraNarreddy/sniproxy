...
func GenerateKeyStore(keyStoreFile *string, hostname *string,
	...
	if *keyStoreFile == "" {
		return fmt.Errorf("keyStore argument not set. Please use -keyStore flag")
	}
	if *pemCertFile == "" {
		return fmt.Errorf("pemCert argument not set. Please use -pemCert to set it")
	}
	if *pemKeyFile == "" {
		return fmt.Errorf("pemkey argument not set. Please use -pemkey to set it")
	}
	if *keyStorePass == "" {
		fmt.Printf("keyPass argument is not set. Proceed with blank password?[y/anyotherkey]: ")
	...
	case *ecdsa.PublicKey:
		...
		certType = "ECDSA"
	default:
		log.Fatal(errors.New("Sniproxy error - Unsupported public key algorithm"))
	}
	...
	if fileExists(*keyStoreFile) {
		...
		if loadKSErr != nil {
			log.Fatal(errors.New("Sniproxy error - keyStore file loading failed with the error: " +
