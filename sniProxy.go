...
//MinTLSVer is the minimum version of TLS that Sni enforces for client
...
func SniProxy(keyStoreFile *string, keyStorePass *string,
	...
	assignRoutes(&pHMap, routeMap, authChecker)
	...
	go func(sigChannel <-chan os.Signal) {
		for {
			...
			case <-sigChannel:
				...
				clearOut(RSAdefault)
				log.Printf("Sniproxy error - Received %#v, purged keystore secret and certificate map. Goodbye!\n", sigChannel)
				os.Exit(1)
			}
		}
	}(sigChannel)
